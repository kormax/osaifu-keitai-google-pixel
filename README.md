...
Google Pixel series starting from 4 and 6 have the required FeliCa applet provisioned into the Secure Element (SE) from the factory regardless of target region. As for other models, they have to be verified separately.
...
c) Otherwise, it retrieves [ContentProvider](https://developer.android.com/guide/topics/providers/content-providers) URL from key `00000014` and column index `00000015`. Application queries the provider, if it returns `1` result is considered a success.  
...
In conclusion, **this app actually makes your device worse**. It's not responsible for proper NFC operation in any way, unlike what the name may lead people to think. Its only purpose is to tell Osaifu-Keitai app if it should or shouldn't let you in.
...
1. Manipulate `com.google.android.pixelnfc`:  
  (Failure) System apps are well-protected against manipulation.
    ...
    (Failure) You cannot install a new application that declares a provider with conflicting URL until you uninstall an old one.  
...
> **Warning**  
> Following solutions should ONLY be attempted if you know what you are doing as it is possible to PERMANENTLY render your device unusable.  
...
For more details, read [this forum topic at XDA](https://forum.xda-developers.com/t/converting-japanese-pixel-6-to-global-version.4365275/).  
  This solution has following upsides:
    ...
    I lack required expertise to fully assess safety and correctness of that script, so the only assurance in this case is the high forum reputation of its creator.  
    There are no implications it's malicious, but if you're uncomfortable with this fact, it's advised to look at the other two solutions. (This is the reason why I did the other ones, personally).
...
This solution has the same upsides and downsides as 2), although more complex to replicate.
...
4. To unpack and pack apk into modifiable [SMALI](https://github.com/google/smali) form, use [apktool](https://ibotpeaches.github.io/Apktool/);
5. To verify that your SMALI modifications did not break the code, you can decompile the app into java source code using [jadx](https://github.com/skylot/jadx) and check that the modified code has no errors; 
...
8. Some apps detect root by trying to invoke Magisk. In this case you have to hide Magisk and add the problematic app into the denylist;
...
- In case you've been able to re-create the provided steps, feel free to report your success and findings through the proccess;  
...
- Tools or websites to download APK files with:
  ...
  Violates Google's TOS, strongly advised not to use personal Google account with it, instead try anonymous mode or a throwaway account;
